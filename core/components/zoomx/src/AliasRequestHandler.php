...
class AliasRequestHandler extends RequestHandler
{
    ...
    public function getResourceIdentifier()
    {
        ...
        if (!file_exists(ZOOMX_CONFIG_PATH . 'routes.php')) {
        ...
        // Подготавливаем uri ресурса для другого контекста-папки
        if ($this->modx->context->key !== 'web' && $uri !== $this->modx->getOption('base_url')) {
            $uri = str_replace($this->modx->getOption('base_url'), '', $uri);
        }

        return $this->modx->resourceIdentifier ?? ltrim($uri, '/');
    }
