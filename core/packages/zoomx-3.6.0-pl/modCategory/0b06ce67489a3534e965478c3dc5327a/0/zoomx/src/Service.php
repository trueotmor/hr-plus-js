...
class Service
{
    ...
    private function __construct(modX $modx)
    {
        ...
        define('ZOOMX_CONFIG_PATH', $this->getConfigPath());
    ...
    {
        if (PHP_SAPI  !== 'cli' && (!defined('MODX_API_MODE') || !MODX_API_MODE)) {
            if ($this->modx->context->key !== 'mgr') {
                $this->modx->request = $this->shouldBeJson() ? $this->getJsonRequest() : $this->getRequest();
                if ($this->modx->getOption('zoomx_parse_json_to_post', null, false)) {
                    $this->parseJsonToPost();
                }
            }
    ...
    /**
     * Determines if the given content should be turned into JSON
     * @return bool
     */
    public function shouldBeJson()
    {
        return preg_match('/(\/json|\+json)/', $_SERVER['HTTP_ACCEPT'] ?? '') || $this->isJson();
    }
    ...
    {
        return preg_match('/(\/json|\+json)/', $_SERVER['CONTENT_TYPE'] ?? '');
    }
    ...
    private function parseJsonToPost()
    {
        if ('POST' === ($_SERVER['REQUEST_METHOD'] ?? 'GET') && $this->isJson()) {
